...
import { Search, Plus, Minus, Trash2, ShoppingCart, Package, UserCircle, X, Star } from "lucide-react";
...
export function PosClient({
  ...
  return (
    <div className="grid lg:grid-cols-[1fr_420px] gap-4 h-[calc(100vh-8rem)]">
      {/* ── Product grid ── */}
            ...
            return (
              ...
              >
                {p.imageUrl ? (
                  ...
                  <div className="w-full h-24 bg-primary-container/10 flex items-center justify-center">
                    <span className="text-2xl font-bold text-primary/30 font-display">{p.name.charAt(0).toUpperCase()}</span>
                  </div>
                )}
                <div className="p-3 w-full flex-1 flex flex-col">
                  <div className="text-[11px] text-muted-foreground">{p.sku}</div>
                  <div className="font-medium text-sm line-clamp-2 flex-1 text-on-surface">{p.name}</div>
                  <div className="mt-1.5 flex items-center justify-between w-full">
                    <div className="font-bold text-on-primary-container">{formatMoney(p.price)}</div>
                    {out ? (
                      <span className="text-[11px] text-destructive font-medium">{t("outOfStock")}</span>
                    ) : low ? (
                      <span className="text-[11px] text-amber-500">{p.stock} {tu(p.unit)}</span>
                    ) : (
                      <span className="text-[11px] text-muted-foreground">{p.stock} {tu(p.unit)}</span>
                    )}
                  </div>
                </div>
              </button>
      ...
      {/* ── Cart panel ── */}
      <Card className="flex flex-col min-h-0 bg-surface-lowest">
        {/* Cart header */}
